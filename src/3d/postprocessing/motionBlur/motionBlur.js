...
function init(sampleCount) {
  ...
  _motionRenderTarget = fboHelper.createRenderTarget(
    1,
    1,
    THREE.RGBAFormat,
    THREE.FloatType
  ...
  _linesRenderTarget = fboHelper.createRenderTarget(
    1,
    1,
    THREE.RGBAFormat,
    THREE.FloatType
  ...
  _linesMaterial = new THREE.RawShaderMaterial({
    ...
    fragmentShader:
      fboHelper.rawShaderPrefix + glslify("./motionBlurLines.frag"),
...
function resize(width, height) {
  ...
  if (!exports.useSampling) {
    ...
    var amount = noDithering
      ? linesWidth * linesHeight
      : _getDitheringAmount(linesWidth, linesHeight);
    ...
    for (i = 0; i < size; i++) {
      ...
      if (noDithering || (x + (y & 1)) & 1) {
        _linesPositions[i6 + 0] = _linesPositions[i6 + 3] =
          (x + 0.5) / linesWidth;
        _linesPositions[i6 + 1] = _linesPositions[i6 + 4] =
          (y + 0.5) / linesHeight;
...
function render(dt, renderTarget, toScreen) {
  ...
  if (!useSampling) {
    ...
    _linesMaterial.uniforms.u_motionMultiplier.value =
      exports.motionMultiplier * fpsRatio;
    _linesMaterial.uniforms.u_depthTest.value = exports.depthTest;
    _linesMaterial.uniforms.u_opacity.value = exports.opacity;
    _linesMaterial.uniforms.u_leaning.value = Math.max(
      0.001,
      Math.min(0.999, exports.leaning)
    );
    _linesMaterial.uniforms.u_depthBias.value = Math.max(
      0.00001,
      exports.depthBias
    );
    ...
    effectComposer.renderer.render(
      _linesScene,
      _linesCamera,
      _linesRenderTarget
    );
  ...
  if (useSampling) {
    ...
    _samplingMaterial.uniforms.u_motionMultiplier.value =
      exports.motionMultiplier * fpsRatio;
    _samplingMaterial.uniforms.u_leaning.value = Math.max(
      0.001,
      Math.min(0.999, exports.leaning)
    );
